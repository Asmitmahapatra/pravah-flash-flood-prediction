...
# --- HEADER ---
st.title("🌊 PRAVAH — Flash-Flood Early Warning & Intelligence Dashboard")
st.caption("AI-Powered Multi-Source Spatio-Temporal Prediction for the Maharashtra Western Ghats")

# --- SIDEBAR ---
with st.sidebar:
    st.image("https://img.icons8.com/color/96/flood.png", width=70)
    st.header("⚙️ Control & Configuration")
    ...
    st.divider()
    st.info("💡 **Study Region:** 20 Open Gauge Stations across the 100 km Maharashtra Western Ghats corridor proxy.")
...
if selected_view == "🗺️ Live Risk & Catchment Map":
    st.subheader("Interactive Catchment Risk Map & Live Rainfall Simulator")
    
    col_map, col_sim = st.columns([1.6, 1.2])
    
    # Rainfall presets
    PRESETS = {
        "Custom Sliders": None,
        "Dry Season (0 mm/day)": [0.0] * 10,
        "Moderate Monsoon Saturation (15–40 mm/day)": [5.0, 10.0, 15.0, 20.0, 25.0, 35.0, 40.0, 30.0, 25.0, 35.0],
        "Heavy Ghats Storm Surge (60–120 mm/day)": [15.0, 25.0, 45.0, 70.0, 95.0, 120.0, 140.0, 110.0, 85.0, 95.0],
        "Extreme Catastrophic Deluge (150–300 mm/day)": [40.0, 80.0, 120.0, 180.0, 240.0, 310.0, 280.0, 220.0, 190.0, 210.0],
    }
    
    with col_sim:
        st.markdown("### 🌧️ Rainfall Scenario Simulator")
        ...
        selected_gauge = st.selectbox("Select Target Catchment", gauge_list, format_func=lambda x: gauge_labels[x], index=19)
        
        preset_choice = st.selectbox("Quick Rainfall Preset", list(PRESETS.keys()), index=0)
        
        rainfall_values = []
        if preset_choice != "Custom Sliders" and PRESETS[preset_choice] is not None:
            rainfall_values = PRESETS[preset_choice]
            st.write(f"**Applied Preset Rainfall Series (Days T-10 to T-1):**")
        ...
        st.divider()
        st.markdown(f"### Current Alert Level: **:{color.lower()}[{ALERT_EMOJI_MAP.get(tier, tier)}]**")
        st.info(f"**Action Recommendation:** {rec}")
        
        kpi1, kpi2 = st.columns(2)
        kpi1.metric(
            label="Onset Risk (1-Day Ahead)",
            value=f"{pred_res['task_a_onset']['probability']:.1%}",
            delta="HIGH RISK" if pred_res['task_a_onset']['is_flood_onset_predicted'] else "NORMAL",
        ...
        kpi2.metric(
            ...
            delta="ACTIVE FLOOD" if pred_res['task_b_active']['is_active_flood_predicted'] else "INACTIVE",
        ...
        fig_rain.update_layout(
            title="10-Day Pre-Cutoff Rainfall Accumulation",
            xaxis_title="Days Before Prediction Cutoff",
    ...
    with col_map:
        st.markdown("### 🗺️ Maharashtra Western Ghats Catchments")
        # Base Folium Map centered on Western Ghats
        m = folium.Map(location=[18.0, 74.3], zoom_start=8, tiles="CartoDB positron")
        ...
        for feature in geojson_data.get("features", []):
            ...
            folium.GeoJson(
                ...
                style_function=lambda x, is_sel=is_selected, p_color=poly_color: {
                    "fillColor": p_color if is_sel else "#95a5a6",
                    "color": "#2c3e50",
                    "weight": 3 if is_sel else 1.2,
                    "fillOpacity": 0.65 if is_sel else 0.25,
                },
                tooltip=f"<b>Station:</b> {info.get('station_name')} (Gauge {gid})<br><b>River:</b> {info.get('river')}<br><b>Danger Level:</b> {info.get('danger_level_m')} m",
            ).add_to(m)
            ...
            if lat > 0 and lon > 0:
                marker_color = "red" if is_selected and tier in ["WARNING", "EMERGENCY"] else ("orange" if is_selected and tier == "ADVISORY" else "blue")
                folium.Marker(
                    location=[lat, lon],
                    popup=f"<b>{info.get('station_name')}</b><br>River: {info.get('river')}<br>Warning: {info.get('warning_level_m')} m",
...
# =========================================================================
    ...
    with col_date:
        event_choice = st.selectbox("Select Benchmark Historical Event", list(HISTORICAL_HIGHLIGHTS.keys()), index=1)
        default_date = HISTORICAL_HIGHLIGHTS[event_choice]
        selected_date = st.text_input("Or Enter Custom Date (YYYY-MM-DD)", value=default_date)
    ...
    with col_summary:
        st.markdown(f"### Simulation Overview for **{selected_date}**")
        k1, k2, k3, k4 = st.columns(4)
        k1.metric("Active Stations", sim_data["total_stations_active"])
        k2.metric("🔴 Emergency Alerts", sim_data["emergency_count"])
        k3.metric("🟠 Warnings", sim_data["warning_count"])
        k4.metric("🟢 Normal Stations", sim_data["normal_count"])
    
    st.divider()
    st.markdown("### Catchment-Level Prediction vs. CWC Ground Observation")
...
# =========================================================================
    ...
    st.subheader("Machine Learning Performance Benchmarks & Feature Importance")
    ...
    with tab_a:
        ...
        st.caption("Chronological Out-of-Sample Test Evaluation on Unseen Years 2016–2020")
        ...
        if rf_feats:
            ...
            fig = px.bar(
                ...
                labels={"importance": "Gini Feature Importance", "feature_name": "Predictor Feature"},
                color="importance",
    ...
    with tab_b:
        ...
        if xgb_feats:
            ...
            fig_b = px.bar(
                ...
                y="feature_name",
                orientation="h",
                title="XGBoost Top 15 Feature Importances (Task B: Active Flood State)",
                labels={"importance": "Normalized Gain Importance", "feature_name": "Predictor Feature"},
                color="importance",
...
# =========================================================================
    ...
    st.subheader("Catchment Morphometric, Terrain & Socio-Economic Profiles")
    ...
    if chars_path.exists():
        ...
        gauge_choice = st.selectbox(
            "Select Catchment for Full Profile",
